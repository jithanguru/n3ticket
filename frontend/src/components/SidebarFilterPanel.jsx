...
import { Film, Bus, Plane, Trophy, Activity, Ticket, MapPin, Calendar, Percent, RefreshCw, Filter, ChevronDown, ChevronUp } from 'lucide-react';
...
export function SidebarFilterPanel() {
  ...
  return (
    <aside className="sidebar-panel">
      {/* Header Bar with Mobile Toggle Support */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <button
          onClick={() => setMobileOpen(!mobileOpen)}
          style={{ background: 'none', border: 'none', color: '#fff', display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', padding: 0 }}
        >
          <Filter size={18} color="var(--primary)" />
          <h3 style={{ fontSize: '1.05rem', fontWeight: 600, color: '#fff', margin: 0 }}>Filter Listings</h3>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-dim)', marginLeft: '0.2rem' }} className="mobile-only-inline">
            ({selectedCategory === 'all' ? 'All' : selectedCategory})
          </span>
          <span className="mobile-only-inline" style={{ marginLeft: '0.2rem' }}>
            {mobileOpen ? <ChevronUp size={16} color="var(--text-muted)" /> : <ChevronDown size={16} color="var(--text-muted)" />}
          </span>
        </button>
      ...
      {/* Filter Body Content (Always visible on Desktop/Tablet, Collapsible on Mobile) */}
      <div className={`sidebar-body ${mobileOpen ? 'open-mobile' : ''}`} style={{ marginTop: '1.25rem' }}>
        {/* 1. Category Panel */}
