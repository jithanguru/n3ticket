...
export function Navbar({ onOpenSellModal, onOpenAuthModal, onOpenDashboard, onOpenProfile, onOpenNotifications, onOpenMessenger, unreadCount }) {
  ...
  const { liveNotification, clearNotification } = useSocket();
  ...
  return (
    <>
      ...
      <div className="ticker-bar">
        ...
        <span style={{ fontWeight: 600, color: '#fff' }}>LIVE STREAM:</span>
        <div style={{ flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {liveNotification ? (
            <span style={{ color: 'var(--primary)', fontWeight: 600 }}>
              {liveNotification.title} {liveNotification.message} ({liveNotification.timestamp})
              <button onClick={clearNotification} style={{ background: 'none', border: 'none', color: '#fff', marginLeft: '10px', cursor: 'pointer', textDecoration: 'underline' }}>Dismiss</button>
            </span>
          ) : (
            <span>🚀 100% Guaranteed Authentic Tickets • Verified Resale • Instant Digital Transfer</span>
          )}
        </div>
      </div>
    </>
